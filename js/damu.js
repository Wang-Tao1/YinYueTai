...
(function(w) {
	...
	w.damu.carousel = function(arr) {
		...
		if (carouselWrap) {
			...
			carouselWrap.addEventListener("touchmove", function(e) {
				...
				if (!needCarousel && index == 0 && disX > 0 && Math.abs(disX) > 20) {
					return
				} else if (!needCarousel && index == -(pointslength - 1) && disX < 0 && Math.abs(disX) > 20) {
					return
				}
	...
	w.damu.vMove = function(wrap, callBack) {
		...
		var start={};
		var element ={};
		console.log(wrap.clientHeight,item.offsetHeight);
